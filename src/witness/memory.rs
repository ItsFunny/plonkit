...
use std::fmt::Error;
use ark_ff::{BigInteger, BigInteger256, FpParameters, FromBytes};
...
impl SafeMemory {
    ...
    pub fn new(memory: Memory, n32: usize, prime: BigInt) -> Self {
        ...
        let short_min = BigInt::from_biguint(
            ...
            BigUint::from_bytes_le(&FrParameters::MODULUS.to_bytes_le()),
        ) - &short_max;
    ...
    pub fn read_big(&self, ptr: usize, num_bytes: usize) -> Result<BigInt> {
        ...
        let big = BigUint::from_bytes_le(&big.to_bytes_le());
        Ok(big.into())
    }
}
...
fn try_from(val: num_bigint::BigUint) -> Result<BigInteger256, String> {
    let bytes = val.to_bytes_le();
    if bytes.len() > (4 * 8) as usize {
        Err(format!(
            "A BigUint of {} bytes cannot fit into ",
            bytes.len()))
    } else {
